...
root.config(bg='#96CDCD')
Tops = Frame(root,width = 1600,height=50,relief=SUNKEN)
Tops.pack(side=TOP)
bottoms = Frame(root,width = 1600,height=50,relief=SUNKEN)
...
lblinfo = Label(Tops, font=('Times New Roman',30, 'bold'),text="Restaurant Management System By Suresh",fg="black",bd=10,anchor='w')
lblinfo.grid(row=0,column=0)
lblinfo = Label(Tops, font=( 'aria',15 ),text=currenttime,fg="black",anchor=W)
lblinfo.grid(row=1,column=0)
print(currenttime)
lblinfo = Label(bottoms, font=( 'aria' ,20, 'bold' ),text="contact us:+91 6379991294",fg="black",bd=10,anchor='w')
lblinfo.grid(row=0,column=0)
lblinfo = Label(bottoms, font=( 'aria' ,20, ),text='mail:abc@gmail.com',fg="black",anchor=W)
lblinfo.grid(row=1,column=0)
...
status = Label(f2,font=('aria', 15, 'bold'),width = 16, text="-By Suresh",bd=2,relief=SUNKEN)
...
def Ref():
    ...
    cof= float(Fries.get())
    colfries = float(Largefries.get())#lunch meal
    ...
    fileexist=os.path.isfile('hotel.csv')
    if fileexist:
        with open('hotel.csv','a',newline='') as file:
            writer=csv.writer(file)
            writer.writerow([randomRef, cof, colfries, cob, cofi, cochee, codr, costoffries, costoflargefries, costofburger,
                 costoffilet, costofcheeseburger, costofdrinks, costofmeal, PayTax, Totalcost, Ser_Charge, Service,
                 OverAllCost, PaidTax])
    else:
        with open('hotel.csv', 'a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(
                ['randomRef','cof','colfries','cob,cofi','cochee,codr','costoffries','costoflargefries','costofburger','costoffilet','costofcheeseburger','costofdrinks','costofmeal','PayTax','Totalcost','Ser_Charge','Service','OverAllCost','PaidTax'])
            writer.writerow(
